...
## Pipeline
`/ponder` (interactive) → `/forge` (autonomous dispatch loop) → `/hammer <N>` (subagent per slice, max 2 concurrent)
...
`/hammer <N>` per slice: setup → build → verify → PR → CI (Monitor, zero cost) → merge

## Context discipline
- Hammer subagents: **40% context = warning** (wrap up current phase), **50% = hard stop** (write continuation, hand off)
...
| `/hammer <N>` merges | Done | Auto (issue close) |

## Hammer sentinels
- `HAMMER:SUCCESS` — slice merged, forge logs tokens and moves to next
- `HAMMER:CONTINUE:<N>` — context overflow, forge reads continuation file and spawns fresh session
- `HAMMER:NEEDS_HUMAN:<reason>` — stuck, forge notifies and skips
- `HAMMER:FAIL:<reason>` — forge retries once, then marks needs-human

## Friction protocol
Hit friction → add `friction` label to PR → post comment with details → if unresolved: `HAMMER:NEEDS_HUMAN:friction`
Forge reviews friction-labelled PRs at end of batch and updates lessons.md for recurring patterns.

## Token tracking
Forge logs per-hammer correlation data to `.claude/token-usage.jsonl`. Analysis via ccusage.
