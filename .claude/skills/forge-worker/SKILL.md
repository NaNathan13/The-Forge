---
name: forge-worker
description: Build a triaged slice end-to-end — branch, implement, test, PR, green CI. Stops at green CI and emits FORGE:RESULT; does NOT merge (that's /seal's job). Invoked as /forge-worker <N> where N is the issue number; usually dispatched by /forge during a batch.
---

# /forge-worker — Build a Slice

Build issue #<N> from branch to merged PR. The forge worker is context-disciplined:
start lean, stay lean, hand off to fresh sessions when context grows.
