...
6. **Recompute the Recommended next prompt.** Priority order — first match wins:
   ...
   1. **Forge in progress:** if any row is `🚧 in-progress` AND has open issues with `ready-for-agent` + `slice:*`, write `/forge <lowest-open-issue>`.
   2. **Ready to forge:** else if any issue has `ready-for-agent` + `slice:*`, write `/forge <lowest-such-issue>`.
   3. **PRD ready:** else if any row is `📝 prd-ready` with issues filed, write `/forge <N>`.
