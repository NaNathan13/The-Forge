---
name: temper-worker
description: Review and harden a built slice — dispatches the reviewer agent on the diff, runs an inline intent-match against the issue body, and applies a strict friction rule (any reviewer HIGH or intent-match failure → friction; else ready-for-seal). Invoked as /temper-worker <N>.
---

# /temper-worker — Review and Harden a Built Slice

`/temper-worker` is the per-PR worker of the [Temper phase](../../../CONTEXT.md#temper).
It runs **after** `/forge-worker` produces a green-CI PR for a slice. The pipeline shape:
...
[`/temper`](../../../CONTEXT.md#temper) — that dispatches
...
1. **Resolve the PR.** Fetch the issue number from the argument; if
   `/temper` passed the PR number on `FORGE:RESULT`, use it directly.
...
3. **CI is green.** `gh pr checks <PR>` — last run must be green. If not green, this is
   ...
   `TEMPER:RESULT` with `status:"needs_human"`, `reason:"ci-not-green"`. (`/forge`'s
...
(`subagent_type: "general-purpose"` is the project convention — see `.claude/skills/forge-worker/SKILL.md` for the matching `/forge-worker` dispatch context; `.claude/skills/forge-worker/SKILL.md` §Rules has the canonical support-agent dispatch protocol. The `reviewer.md` file is an **agent definition** loaded into the prompt, not a `subagent_type` value.)
...
`TEMPER:RESULT` JSON line. The JSON is the source of truth `/temper` parses; the
...
line so `/temper` can parse it deterministically.
...
- `tokens` — always `null` from `/temper`. `/temper` fills this in via ccusage
  after the run.
...
green. The sentinel tells `/temper` to skip to the next PR in *this* batch; the
label tells `/seal` to skip the PR at close-out. Both signals are required.

## Continuation files

If `/temper-worker` ever needs to hand off mid-run (rate-limit pressure, context, etc.) it
writes `.claude/temper-continue-<N>.md` in the hardened five-section format described
in `.claude/skills/forge-worker/SKILL.md` §"Continuation file format". The typical `/temper-worker`
run is small enough that this should rarely trigger; the format alignment exists so a
resuming `/temper-worker` inherits a known shape.
