---
name: foundry
description: The forge master — orchestrates the full execution lifecycle (build, test, CI, merge) by dispatching and overseeing forge workers. Invoked as /foundry after ponder has triaged all slices.
---

# Foundry — The Forge Master

Foundry is an **autonomous dispatch loop**. It pulls slices from the build queue, dispatches
forge workers as fresh subagents, monitors their progress, handles results, and moves to the
next slice — repeating until the queue is drained or the user intervenes.

Ponder plans the work; foundry executes it. Each forge handles one slice end-to-end:
build → test → PR → CI → merge.

## Invocation

```
/foundry                    # resume from current ready-for-agent backlog
/foundry --phase <id>       # scope to one sub-phase (e.g. 2a)
```
...
2. Dispatch forge as a subagent:
   ```
   Agent({
   ...
   ```
3. Max 2 concurrent forge workers. Wait for one to complete before dispatching a third.
4. On forge completion, handle the sentinel (see below)
...
| Sentinel | Foundry action |
|----------|---------------|
| `FORGE:SUCCESS` | Log tokens, move to next slice |
| `FORGE:CONTINUE:<N>` | Read `.claude/forge-continue-<N>.md`, dispatch fresh forge with continuation context |
| `FORGE:NEEDS_HUMAN:<reason>` | Log the reason, notify user, skip to next slice |
| `FORGE:FAIL:<reason>` | Retry once with fresh session. If second failure, mark `needs-human`, skip |

## Context Discipline

Context bloat is the #1 cost driver. Every session — foundry and forge — should stay lean.

### Forge subagent limits
- **40% context — warning.** Forge should finish its current phase and evaluate handoff.
- **50% context — hard stop.** Write continuation file, emit `FORGE:CONTINUE:<N>`.
- Forge workers start fresh (worktree isolation) and load only the issue + auto-loaded rules.
  No bulk-loading of lessons.md, MISSION-CONTROL.md, or WORKFLOW.md at startup.
- If CI fails after PR is opened, foundry dispatches a **fresh subagent** with just the
  branch name, PR number, and failure log — not the full build context.

### Foundry self-limits
- **40% context — start fresh.** Write `.claude/foundry-continue.md` with queue state,
...
- **No forced model.** Forge workers inherit the session's model (typically Opus). Don't
  downgrade to Sonnet — it causes more retries and wastes more tokens than it saves.
- **Poll sub-agents actively.** Check on running forge workers every ~30s. Don't go silent
  while a subagent runs — the user should see progress updates.
- **Milestone reporting.** Forge workers communicate progress at key phases:
  after setup, after build, after tests pass, after PR opens, after CI completes, after merge.
  Foundry relays these milestones to the user.
- **Lean context loading.** Forge workers read only the issue and auto-loaded rules.
  Everything else is reactive — read it when you need it, not at startup.
- **Research via skills.** If a forge worker needs to look something up, use
...
## Rules
- Foundry is an autonomous loop — dispatch, handle, loop. No pause between slices.
- Max 2 concurrent forge subagents
- Always present build queue before dispatching — never skip user approval
- Token logging is foundry's responsibility, not forge's
