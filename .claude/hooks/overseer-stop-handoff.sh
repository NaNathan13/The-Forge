...
# overseer-stop-handoff.sh — P2 Stop hook: handoff enforcer + heartbeat.
#
...
# SessionStart hook (overseer-session-start.sh), on each launch, records a baseline
...
# carries it. This hook enforces the handoff ONLY when OVERSEER_LOOP_MANAGED is set
...
main() {
  ...
  # OVERSEER_LOOP_MANAGED is the explicit "this generation is loop-managed" marker
  ...
  if [[ -z "${OVERSEER_LOOP_MANAGED:-}" ]]; then
    allow_stop
  fi
