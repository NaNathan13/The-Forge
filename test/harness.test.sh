...
test_assert_contains_matches_substring() {
  assert_contains "the OVERSEER_CONTINUE sentinel" "OVERSEER_CONTINUE"
}

test_assert_not_contains_rejects_substring() {
  if assert_not_contains "has OVERSEER_COMPLETE here" "OVERSEER_COMPLETE" 2>/dev/null; then
...
test_stub_result_carries_forge_continue_sentinel() {
  local out result
  out="$(CLAUDE_STUB_RESULT="handed off cleanly OVERSEER_CONTINUE" "$CLAUDE_STUB")"
  result="$(jq -r '.result' <<<"$out")"
  assert_contains "$result" "OVERSEER_CONTINUE"
}

test_stub_result_carries_forge_complete_sentinel() {
  local out result
  out="$(CLAUDE_STUB_RESULT="work done OVERSEER_COMPLETE" "$CLAUDE_STUB")"
  result="$(jq -r '.result' <<<"$out")"
  assert_contains "$result" "OVERSEER_COMPLETE"
}
...
test_stub_loads_fixture_file() {
  ...
  fixture="$(mktemp)"
  cat > "$fixture" <<'EOF'
CLAUDE_STUB_RESULT="from fixture OVERSEER_CONTINUE"
...
EOF
  ...
  rm -f "$fixture"
  assert_contains "$(jq -r '.result' <<<"$out")" "OVERSEER_CONTINUE"
