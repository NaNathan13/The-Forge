...
test_relaunches_on_clean_handoff_sentinel() {
  # Drive the loop with a stub that emits FORGE_CONTINUE, assert it relaunched.
...
|---|---|---|
| `CLAUDE_STUB_RESULT` | the `.result` string — **put sentinel strings here** (`FORGE_CONTINUE` / `FORGE_COMPLETE`) | `"stub result"` |
...
```bash
export PATH="$TEST_DIR/stubs:$PATH"
CLAUDE_STUB_RESULT="...FORGE_COMPLETE" bash "$REPO_ROOT/scripts/relaunch-loop.sh"
```
...
| Fixture | Scenario |
|---|---|
| `clean-handoff-under-budget.sh` | `FORGE_CONTINUE`, usage under the warn line → relaunch normally |
| `clean-handoff-over-hard.sh` | `FORGE_CONTINUE`, usage past the hard line → loop must not start another generation past hard without a handoff |
| `work-complete.sh` | `FORGE_COMPLETE`, exit 0 → loop breaks |
