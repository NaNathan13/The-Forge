...
teardown() {
  rm -rf "$WORKDIR"
  unset FORGE_DIR FORGE_LOOP_MANAGED FORGE_RETENTION_CAP
}
...
- approved-queue: true
- One temper per generation; forge does not self-measure context.
...
dispatch temper for issue #95

## Notes / scratch

gen-001 written before the first temper — crash defence per PRD Q3.
EOF
}
...
test_resumed_generation_carries_next_concrete_action() {
  ...
  assert_contains "$ctx" "dispatch temper for issue #95" \
