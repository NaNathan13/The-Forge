#!/usr/bin/env bash
# temper-continuation.test.sh — tests for /forge's continuation file format
# (issue #185, slice 1c-5).
#
# forge/SKILL.md was updated so /forge's continuation file
# (.claude/forge-continue-<N>.md) is reformatted into the hardened five-section
# `gen-NNN.md` schema that the P2 substrate uses (templates/continuation-gen.md).
# This is a FORMAT alignment only: /forge stays a subagent — it does NOT move to
# the `.forge/continuation/<slug>/` chain and does NOT call continuation.sh.
#
# forge/SKILL.md is a markdown skill, not a shell script, so it has no direct
...
test_skill_documents_all_five_section_headings() {
  ...
  # appear as headings in /forge's continuation-file format block.
  local body
  body="$(cat "$SKILL")"
  assert_contains "$body" "## Hard constraints (RESTATED VERBATIM — do not summarize)" \
    "forge/SKILL.md must document the verbatim Hard constraints section"
  assert_contains "$body" "## Execution frontier" \
    "forge/SKILL.md must document the Execution frontier section"
  assert_contains "$body" "## Conversation summary" \
    "forge/SKILL.md must document the Conversation summary section"
  assert_contains "$body" "## Next concrete action" \
    "forge/SKILL.md must document the Next concrete action section"
  assert_contains "$body" "## Notes / scratch" \
    "forge/SKILL.md must document the Notes / scratch section"
}
...
test_skill_section_headings_match_the_gen_template() {
  ...
  while IFS= read -r heading; do
    assert_contains "$skill_body" "$heading" \
      "forge/SKILL.md must use the gen-NNN.md heading verbatim: $heading"
  done < <(grep -E '^## ' "$GEN_TEMPLATE")
}

test_skill_keeps_per_issue_continuation_path() {
  # Format-only reformat — the file stays at .claude/forge-continue-<N>.md so
  # forge/seal/scrub references do not break.
  assert_contains "$(cat "$SKILL")" ".claude/forge-continue-<N>.md" \
    "forge/SKILL.md must keep the per-issue .claude/forge-continue-<N>.md path"
}
...
test_skill_keeps_temper_outside_the_forge_chain() {
  ...
  assert_contains "$body" ".forge/continuation/<slug>/" \
    "forge/SKILL.md must name the .forge chain it deliberately does NOT join"
  assert_contains "$flat" "does not call \`scripts/continuation.sh\`" \
    "forge/SKILL.md must state temper does not call scripts/continuation.sh"
}
...
test_skill_documents_format_only_no_behavior_change() {
  ...
  assert_contains "$flat" "format alignment only" \
    "forge/SKILL.md must state the change is a format alignment only"
}

test_skill_documents_the_read_side() {
  # Acceptance criterion: SKILL.md updated to write AND read the new format.
  # The Setup step must tell a resuming /forge to read the continuation file.
  local flat
  flat="$(tr '\n' ' ' < "$SKILL" | tr -s '[:space:]' ' ')"
  assert_contains "$flat" "If resuming from a continuation file" \
    "forge/SKILL.md Setup must tell a resuming /forge to read the continuation file"
}
