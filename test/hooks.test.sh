...
#
# Covers .claude/hooks/overseer-stop-handoff.sh and .claude/hooks/overseer-session-start.sh:
...
#   - Both hooks key off the OVERSEER_LOOP_MANAGED env marker (issue #181): the
...
# dir so nothing leaks between tests. OVERSEER_LOOP_MANAGED is exported in setup so
...
STOP_HOOK="$REPO_ROOT/.claude/hooks/overseer-stop-handoff.sh"
...
setup() {
  ...
  # OVERSEER_LOOP_MANAGED=1 into each `claude -p` generation, and that is the
  # common case the hooks were written for. Tests that exercise an interactive
  # session unset it explicitly (see the OVERSEER_LOOP_MANAGED section).
...
teardown() {
  rm -rf "$WORKDIR"
  unset FORGE_DIR FORGE_RETENTION_CAP OVERSEER_LOOP_MANAGED
}
...
# ── OVERSEER_LOOP_MANAGED marker: loop-managed vs interactive (issue #181) ──────
# The relaunch loop exports OVERSEER_LOOP_MANAGED=1 into every `claude -p`
