#!/usr/bin/env bash
# hooks.test.sh — tests for the P2 Stop + SessionStart hooks (issue #140).
#
...
# dir so nothing leaks between tests.
#
...
test_stop_hook_allows_when_no_baseline() {
  # No .genbaseline → session is not loop-managed (a hand-run interactive
  # session). The hook must ALLOW — never wedge a hand-run session.
  local out
  out="$(stop_input | bash "$STOP_HOOK" 2>/dev/null)"
  assert_not_contains "$out" '"decision":"block"' \
    "a non-loop-managed session must not be blocked"
}
