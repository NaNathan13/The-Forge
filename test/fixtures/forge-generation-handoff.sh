...
# emits OVERSEER_CONTINUE as its final .result line.
#
...
# Loop step 7 instructs. The relaunch loop should then see OVERSEER_CONTINUE, run its
# thrash + budget gates, and relaunch fresh.
#
# FORGE_DIR / a --slug are supplied by the test; continuation.sh derives the rest.
CLAUDE_STUB_RESULT="temper #95 success (PR #110, CI green), continuation written OVERSEER_CONTINUE"
CLAUDE_STUB_USAGE='{"input_tokens":35000,"output_tokens":3000,"cache_creation_input_tokens":0,"cache_read_input_tokens":0}'
CLAUDE_STUB_EXIT=0

# The forge generation writes its next gen-NNN.md before emitting OVERSEER_CONTINUE.
