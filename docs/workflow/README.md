...
3. **Build** — Forge runs an autonomous dispatch loop: `/hammer <N>` workers (max 2 concurrent) implement → test → PR → CI → merge
...
| `/forge` | **Execution** — autonomous dispatch loop, monitor, merge, log tokens | hammer, sync-mission-control |

## Other commands

| Command | What it does |
|---------|-------------|
| `/hammer <N>` | Build issue #N end-to-end (usually dispatched by forge, can run standalone) |
...
- Dispatches `/hammer <N>` workers as subagents (max 2 concurrent)
- Polls workers actively and reports progress at milestones
- Handles hammer results: retries failures, spawns continuations, flags stuck slices
...
- **Hammer subagents** start fresh (worktree isolation), load only the issue + auto-loaded rules. Heavy docs (MISSION-CONTROL.md, lessons.md, project-wide design docs) are read reactively, not at startup. Hard stop at 50% context — write continuation file, hand off to a fresh session.
