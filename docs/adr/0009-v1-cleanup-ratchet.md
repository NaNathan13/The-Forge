# ADR 0009 — v1 cleanup ratchet: no phase IDs in living docs, Mission Control restructured to flat state-buckets

**Status:** Accepted
**Date:** 2026-05-17
**Phase:** P4 — Pipeline naming + permissions · sub-phase 4d (v1 cleanup ratchet)

**Source of truth:** [`docs/prds/improvements-4d-v1-cleanup.md`](../prds/improvements-4d-v1-cleanup.md) — the 4d sub-phase PRD that this ADR distills the two coupled cleanup decisions from.

> This ADR is filed as `0009-` to satisfy the strict `max(NNNN) + 1` allocation rule [`/inscribe`](../../.claude/skills/inscribe/SKILL.md) §A0 enforces. Slice C of this sub-phase renumbers surviving ADRs sequentially after deleting ADR-0001 (historical) and ADR-0005 (superseded by 0007); the renumber lands this ADR at the final number **ADR-0007**. Until slice C ships, references to "the v1 cleanup ratchet ADR" point here at `0009-`.

## Context

The project has gone through four numbered phases (P0–P4) and ~30 sub-phases (0a, 0b, 0z, 1a–1c, 2a, 3a–3i, 4a–4e) to reach a stable four-phase pipeline (Ponder → Forge → Temper → Seal) with symmetric per-phase orchestrators. The phase scaffolding was load-bearing while phases were in flight — it tracked what shipped, what came next, and which sub-phase produced which decision. The workflow is now stable; the pipeline-build journey is over.

The phase scaffolding survives across the project as three categories of artifact:

1. **Phase IDs in living-doc prose.** Parenthetical lineage (`(P3 / sub-phase 3g)`), amendment dates (`amended 2026-05-17 in sub-phase 4a`), precedent callouts (`per 4b precedent`), and run-evidence framings (`the 4c run produced empirical evidence`) appear throughout `CLAUDE.md`, `CONTEXT.md`, `WORKFLOW.md`, `README.md`, every `SKILL.md`, every doc under `docs/workflow/` and `docs/shared/`, every `.claude/rules/*.md`, and every `templates/*` file. These were load-bearing during the phases they referenced; they are noise to a reader encountering The Forge as a finished workflow.

2. **`MISSION-CONTROL.md`'s phase-progress ledger.** The `## 🪐 Phase progress` section is the organizing principle of MC — every shipped sub-phase has a row, every phase has a header (`### P0 Foundations ▓▓▓ 3/3`), and the document opens with `**Phase:** P5 — Dev Mode ░ 0/1`. The structure was useful while phases were in flight; once the pipeline is stable, the ledger keeps growing while the live state-information (what's in flight, what's queued) is buried in scattered tables.

3. **Historical PRDs and superseded ADRs.** `docs/prds/` holds 18 PRDs that drove shipped work and now sit as past-tense artifacts. `docs/adr/0001` was already labeled "Now historical"; `docs/adr/0005` was superseded by 0007 the day it shipped. `docs/audit/`, `docs/design/`, and `docs/research/` likewise hold artifacts that drove shipped work and are no longer working reference material.
...
Historical PRDs (all 18 under `docs/prds/`) and the two stale ADRs (0001 historical, 0005 superseded) are deleted as part of the same ratchet. `docs/audit/`, `docs/design/`, and `docs/research/` are deleted as past-purpose artifact directories. `docs/vision/` and `docs/how-the-forge-works.md` survive as forward-direction and onboarding material respectively, with phase IDs scrubbed.

## Rationale

**The phase scaffolding's cost is paid at every reader and at every rename.** The pre-v1 cost of phase IDs was paid by every reader who had to learn "(P3 / sub-phase 3g)" before they could read the surrounding prose, and by every rename pass that had to update phase context across N docs. The cost of the cleanup is paid once. The arithmetic favors the cleanup as soon as the workflow is stable — which it now is.
...
Bypass of one of these conditions is not enough — the structure can still earn its keep when the rare phase-boundary-of-history catch happens, even if most readers pass through unused.

## Consequences

- **Every living doc is rewritten or scrubbed.** The blast radius is the project's documentation surface: `CLAUDE.md`, `CONTEXT.md`, `README.md`, `WORKFLOW.md`, `MISSION-CONTROL.md`, every `.claude/skills/*/SKILL.md` (~25 files), every `.claude/rules/*.md`, every `docs/workflow/*` (4 files), `docs/shared/pipeline.md`, every `docs/vision/*` (4 files), `docs/how-the-forge-works.md`, `.claude/lessons.md`, every `.claude/knowledge/*` file, every `templates/*` file (4 files), and every surviving ADR (6 files).
- **~38 files deleted.** All 18 historical PRDs under `docs/prds/`, ADR-0001, ADR-0005, all of `docs/audit/` (13 files), all of `docs/design/` (4 files), all of `docs/research/` (1 file). The deletes are git-recoverable; their purpose ratchets the v1 break by removing the half-relevant working reference.
- **ADRs renumbered 0001–0006 plus 0007 (this one, post-renumber).** Surviving ADRs (currently 0002, 0003, 0004, 0006, 0007, 0008) renumber sequentially: 0002→0001 (phase isolation), 0003→0002 (concurrency cap), 0004→0003 (context loading defense-in-depth), 0006→0004 (temper review boundary), 0007→0005 (pipeline orchestrator structure), 0008→0006 (naming discipline). This ADR (currently 0009) lands at 0007. All cross-references across the repo updated atomically.
- **One filename rename.** `docs/workflow/p2-resilience-operations.md` is renamed (proposed `docs/workflow/relaunch-loop-operations.md`); every referencing link updated.
...
- **`light-the-forge.sh` ships clean v1 vocabulary to new projects.** The templates mirror slice updates `templates/CLAUDE.md`, `templates/CONTEXT.md`, `templates/MISSION-CONTROL.md` (reshape to flat-ledger empty-state), and `templates/README.md` so a new project initialized with `light-the-forge.sh` starts with the v1 shape rather than the pre-v1 phase scaffolding.
- **ADR-0008's "ADRs and historical PRDs exempt from anchor-link discipline" carve-out is retired.** The carve-out existed because historical PRDs predated the canonical-glossary contract; with all historical PRDs deleted, no exempt class remains. Future PRDs (and surviving ADRs after the rewrite) follow the discipline uniformly.
- **The audit slice's findings doc is ephemeral.** Slice A writes `docs/audit/4d-cleanup-audit.md` per the audit-precedent pattern; slice B deletes `docs/audit/` wholesale. The findings doc lives only across slices A→B and is gone by the time slice B ships. Acceptable: the findings doc's purpose is to guide the cleanup, not to persist as documentation.
...
- **Audit hook.** A future drift audit can grep the repo for sub-phase ID patterns (e.g. `[0-9][a-z]\b` in contexts that look like sub-phase callouts) and flag any reintroduction. The discipline relies on the operator and the agent reviewing diffs; no automated CI gate is added by this ADR.

## Related

- ADR-0008 — [Naming discipline: canonical glossary as single source of truth](./0008-naming-discipline.md) — sibling discipline (canonical-glossary-as-SSOT); the v1 cleanup retires ADR-0008's "ADRs and historical PRDs exempt from anchor-link discipline" carve-out by deleting the exempt class.
- ADR-0007 — [Pipeline orchestrator structure](./0007-pipeline-orchestrator-structure.md) — the structural decision the v1 vocabulary is built around (Ponder → Forge → Temper → Seal with symmetric `<phase>-overseer` orchestrators).
- ADR-0002 — [Phase isolation](./0002-phase-isolation.md) — the architectural commitment whose vocabulary the cleanup preserves under a v1-clean rewrite (becomes ADR-0001 after the renumber).
- PRD — [`docs/prds/improvements-4d-v1-cleanup.md`](../prds/improvements-4d-v1-cleanup.md) — full 4d scope, six slices, per-slice acceptance criteria.
- `MISSION-CONTROL.md` § 🪐 Phase progress — the section deleted by this ratchet's MC reshape (the deletion happens in slice 4 of the 4d PRD).
