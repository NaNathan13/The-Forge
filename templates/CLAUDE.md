...
See [`CONTEXT.md`](./CONTEXT.md) for the full glossary. List the 3-5 most load-bearing
terms here so they're in every session:
...
- [`.forge/README.md`](./.forge/README.md) — single-session resilience substrate (continuation files, `resilience.config`, slug derivation). Read when touching `scripts/continuation.sh` or the `.forge/` runtime dirs.
